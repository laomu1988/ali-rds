...
proto._where = function(where) {
  ...
  const result = this._transWhere(where);
  if (result) {
    debug('_where %j', result);
    return ' WHERE ' + result;
  }
  return '';
};
 ...
 * }
 */
proto._transWhere = function(where, joinOperator) {
  if (!where) {
    return '';
  }
  if (Array.isArray(where)) {
    const array = where.map(data => this._transWhere(data)).filter(v => v)
    if (array.length > 0) {
      return '(' + array.join(' ' + joinOperator + ' ') + ')'
    }
    return '';
  }
  ...
  for(const key in where) {
    ...
    if (key === 'and' || key === 'or' && Array.isArray(value)) {
      wheres.push(this._transWhere(value, key))
    }
    // 假如对象值是Object
      ...
      for(let attr in value) {
        switch(attr) {
          ...
          case '=':
            ...
            childValues.push(value[attr])
            break;
          case 'like':
            childs.push('?? ' + attr + ' \'%' + (SqlString.escape(value[attr]) + '').replace(/['"]/g, '') + '%\'')
            childValues.push(key)
            break;
          default:
            console.error('illegal oprator:' + attr + ', expect >, <, >=, <=, = or like');
            break;
        }
      }
      const result = this.format(childs.join(' AND '), childValues);
      if (result) {
        wheres.push('(' + result + ')')
      }
    }
  ...
  if (wheres.length > 0) {
    return this.format(wheres.join(' AND '), values);
  }
  return '';
}
